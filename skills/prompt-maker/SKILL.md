...
# Prompt Maker Skill — ZORUNLU KULLANIM

**Bu skill her görev başlangıcında zorunlu olarak kullanılmalıdır. Hiçbir istisna yoktur.**

## Kullanım Akışı

### Adım 1: Wiki Bağlam Yükleme
- `.ai/index.md` — Master katalog
- `.ai/brain.md` — Mimari kararlar
- `.ai/keys.md` — Navigasyon haritası

### Adım 2: Web Araştırması
- İlgili konuda minimum 500 kaynak ara
- MDN, RFC, OWASP, resmi dokümanları kontrol et
- Güncel best practice'leri bul

### Adım 3: Soru Döngüsü (Birer Birer)
- Her soruda bir konuya odaklan
- Seçenekler sun (A/B/C/D/E)
- "Diğer" seçeneğini her zaman ekle
- Maksimum 5 soru (bunaltma yok)

### Adım 4: Intent Analizi
- Canonical form oluştur
- Domain tespiti yap
...
### Adım 6: Mimari Karar
- Prompt tipi seçimi (20 tip)
- Output modu seçimi
- Bölüm sayısı (min 20)

### Adım 7: Prompt Üretimi
Minimum 20 bölüm içermeli:
1. System Identity & Role
2. Experience & Expertise Level
3. Project Context & Domain
4. Technical Stack
...
18. Anti-Patterns
19. Best Practices
20. Quality Control & Self-Audit

### Adım 8: Kayıt
- `.ai/wiki/prompts-kb/{dosya}-part{N}.md` — Prompt dosyası
- `.ai/wiki/prompts-kb/{dosya}-index.md` — İndeks
- `.ai/brain.md` — Append
- `.ai/log.md` — Append
...
type: guide
status: active
authority: Prompt Maker Skill
references: [[decisions/NNN-...]]
---
...
1. **Her prompt /prompt-maker ile üretilmeli** — İstisnasız
2. **Web araştırması yapılmalı** — Minimum 500 kaynak
3. **Soru döngüsü çalıştırılmalı** — Kullanıcı onayı alınmalı
4. **20 bölüm zorunlu** — Minimum 20 bölüm
5. **Kayıt yapılmalı** — Brain + log + prompts-kb
6. **Red Team & Truth Mode aktif** — Hallüsinasyon yasak

## İlgili Dosyalar

- `.ai/brain.md` — Merkezi bilgi tabanı
- `.ai/decisions/` — Mimari kararlar
- `.ai/wiki/prompts-kb/` — Üretilen prompt'lar
